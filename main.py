...
class App:
    ...
    def withdraw_money(self):
        ...
        if selected_student:
            try:
                amount = int(self.entry_money_action.get())
                if self.students[selected_student.split(" - ")[0]].balance >= amount:
                    self.students[selected_student.split(" - ")[0]].balance -= amount
                    self.save_students()
                    self.update_students_listbox()
                else:
                    messagebox.showerror("Hiba", "Nincs elég pénz a tanulónál!")
            except ValueError:
                messagebox.showerror("Hiba", "Érvénytelen összeg.")
    ...
    def update_students_listbox(self):
        ...
        for student in self.students.values():
            self.listbox_students.insert(tk.END, f"{student.name} - Egyenleg: {student.balance}")
